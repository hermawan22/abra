...
MCP clients use the existing `propose_learning` tool with `target_type="observation"` and `target_id="<observation-id>"`. Accepted proposals return an apply plan and can be applied only through the explicit `apply_learning_proposal` tool or `POST /learning/proposals/:proposalId/apply`.
...
- `working_memory_compose(task, scope, hook?, agent?, files?, changed_files?, language?, limit?, max_queries?, token_budget?, include_unverified?, diagnostic?, persist_learning?)`
...
`context_window` is the prompt-ready slice of the packet. It packs the safety gate, task, risks, validation, retrieval reasons, summaries, facts, graph relations, impact items, and source excerpts by priority until `token_budget` is reached. The safety gate is kept separate from long task text so memory health, verification, required actions, and the agent decision survive tight context budgets. Sourced blocks include `source_urls` and `citation_refs` so agents can cite the same evidence refs used by the full packet. The gate includes retrieval-quality counters such as result count, canonical unique source count, unsourced result count, dominant source share, unsourced share, low-confidence status, and low-source-diversity status. It returns selected `blocks`, dropped lower-priority blocks, warnings, token estimates, and a rendered `prompt` string. This is how small and large models can consume the same verified memory contract without hand-rolling their own context packing.

From the CLI, `abra compose "ship a change" --scope <scope> --prompt` prints the same decision gate, health signals, validation plan, allowed next actions, and prompt-ready context window for use with any AI client, including clients that do not speak MCP directly. Compose is read-only by default; pass `--persist-learning` only when you want actionable learning suggestions written as pending review proposals and the caller has write access.
...
When `target_type` is `observation`, Abra validates the observation belongs to the same scope, marks it `proposed`, emits `observation.proposed`, and stores a deduplicated pending learning proposal. Accepting that proposal returns a `review_claim_promotion` apply plan; `apply_learning_proposal` or `POST /learning/proposals/:proposalId/apply` performs the explicit promotion step after required approval checks pass.
