...
`context_window` is the prompt-ready slice of the packet. It packs task gate, risks, validation, retrieval reasons, summaries, facts, graph relations, impact items, and source excerpts by priority until `token_budget` is reached. It returns selected `blocks`, dropped lower-priority blocks, warnings, token estimates, and a rendered `prompt` string. This is how small and large models can consume the same verified memory contract without hand-rolling their own context packing.
